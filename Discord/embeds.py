...
class Embeds:
    ...
    def queue(self, songs, current_song, ctx, ctime, curloop) -> discord.Embed:
        ...
        if int(songs[current_song].loop) == 1:
            embed = discord.Embed(title=f"1) {songs[current_song].name}", url=f"{songs[current_song].url}",
                                  description=f"{self.__formatted_line(ctime, songs[current_song].long)}\n`Elapsed: {__time_format1(int(songs[current_song].long - ctime))}`",
                                  color=0x5C3C92)
        else:
            embed = discord.Embed(title=f"1) {songs[current_song].name}", url=f"{songs[current_song].url}",
                                  description=f"{self.__formatted_line(ctime, songs[current_song].long)}\n`Elapsed: {__time_format1(int(songs[current_song].long - ctime))}`\n`Loop: {curloop}{int(songs[current_song.loop])}`",
                                  color=0x5C3C92)
        try:
        ...
        for i in range(current_song + 1, len(songs)):
            if i == len(songs) - 1:
                if int(songs[i].loop) != 1:
                    embed.add_field(name=f"`{i - current_song + 1}) {songs[i].name}`",
                                    value=f"   Duration: `{__time_format1(int(songs[i].long))}`\nLoop: `{int(songs[i].loop)}`",
                                    inline=False)
                else:
                    embed.add_field(name=f"`{i - current_song + 1}) {songs[i].name}`",
                                    value=f"   Duration: `{__time_format1(int(songs[i].long))}`", inline=False)
            else:
                if int(songs[i].loop) != 1:
                    embed.add_field(name=f"`{i - current_song + 1}) {songs[i].name}`",
                                    value=f"   Duration: `{__time_format1(int(songs[i].long))}`\nLoop: `{int(songs[i].loop)}`",
